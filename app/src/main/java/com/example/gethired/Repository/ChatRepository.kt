...
//            roomDatabase.messageRoomDao().delete(messageRoomDto)
    ...
    fun getUserInfo(senderId: Long,receiverId: Long):LiveData<ChattingUserInfo>{
        val userInfo = MutableLiveData<ChattingUserInfo> ()
        val call :Call<ChattingUserInfo> = retrofitAPI.getUserInfo(senderId,receiverId)
        call.enqueue(object: Callback<ChattingUserInfo>{
            override fun onResponse(
                call: Call<ChattingUserInfo>,
                response: Response<ChattingUserInfo>
            ) {
                if(response.isSuccessful){
            ...
            override fun onFailure(call: Call<ChattingUserInfo>, t: Throwable) {
