...
class ChatAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(item.receiver.image!=null){
//            val imageData: ByteArray = Base64.decode(item.image.data, Base64.DEFAULT)
//            val bitmap = BitmapFactory.decodeByteArray(imageData, 0, imageData.size)
//            holder.profileImg.setImageBitmap(bitmap)

        }
        val chatTimeStamp = LocalDateTime.parse(item.timeStamp.substring(0,23), DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS"))
